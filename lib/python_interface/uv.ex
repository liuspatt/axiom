defmodule PythonInterface.Uv do
  @moduledoc false
  ...
  defp python_install_dir(priv?) do
    if priv? do
      Path.join(:code.priv_dir(:python_interface), "uv/python")
    else
  ...
  defp project_dir(pyproject_toml, priv?) do
    if priv? do
      Path.join(:code.priv_dir(:python_interface), "uv/project")
    else
      cache_id =
        pyproject_toml
        |> :erlang.md5()
  ...
  def init(pyproject_toml, priv?) do
    ...
    case :os.type() do
      {:win32, _osname} ->
        ...
        PythonInterface.init(python_dl_path, python_home_path, python_executable_path,
  ...
  defp download!() do
    ...
    archive_binary = PythonInterface.Utils.fetch_body!(url)
