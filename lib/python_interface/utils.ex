defmodule PythonInterface.Utils do
  @moduledoc false
  ...
  def fetch_body!(url) do
    ...
    {:ok, _pid} = :inets.start(:httpc, profile: :python_interface)

    %{scheme: scheme} = URI.parse(url)

    proxy_http_options = set_proxy_options(scheme)

    headers = [{~c"user-agent", ~c"python_interface"}]
  ...
  after
    :inets.stop(:httpc, :python_interface)
  end

  defp request(request, http_options, options) do
    case :httpc.request(:get, request, http_options, options, :python_interface) do
      ...
      when inet in [:inet, :inet6] and
        ...
        :httpc.set_options([ipfamily: fallback_ipfamily(inet)], :python_interface)
        :httpc.request(:get, request, http_options, options, :python_interface)
