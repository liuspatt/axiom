defmodule AxiomAi.LocalModels.Templates do
  ...
  defp get_template(:pythonx_ocr) do
    %{
      ...
      description: "Python-based OCR model"
    }
  end
  ...
  defp get_template(:pythonx_embedding) do
    %{
      ...
      description: "Python-based text embedding model"
    }
  end
  ...
  defp get_template(:pythonx_code) do
    %{
      ...
      description: "Python-based code generation model"
    }
  end
  ...
  # Default Python dependencies for OCR models
  defp default_ocr_dependencies do
    """
    [project]
    name = "axiom_ocr"
    version = "0.1.0"
  ...
  # Default Python dependencies for embedding models
  defp default_embedding_dependencies do
    """
    [project]
    name = "axiom_embedding"
    version = "0.1.0"
    requires-python = "==3.10.*"
    dependencies = [
      "torch >= 2.0.0",
  ...
  # Default Python dependencies for code generation models
  defp default_code_dependencies do
    """
    [project]
    name = "axiom_code_generation"
    version = "0.1.0"
    requires-python = "==3.10.*"
    dependencies = [
      "torch >= 2.0.0",
  ...
  # Default Python inference code for OCR models
  defp default_ocr_inference_code do
    """
    import torch
    from transformers import AutoTokenizer, AutoModelForCausalLM, AutoProcessor
    from PIL import Image
            ...
            if image_path and image_path != "":
                try:
                    if image_path.lower().endswith('.pdf'):
                        image = process_pdf(image_path, 0)
                    else:
                        image = Image.open(image_path).convert('RGB')
  ...
  # Default Python inference code for embedding models
  defp default_embedding_inference_code do
    """
    import torch
    ...
    def load_model(model_path):
        global _model, _tokenizer, _sentence_model, _current_model_path
        ...
        return _tokenizer, _model, _sentence_model
    ...
    def generate_response(model_path, prompt, max_tokens=512, temperature=0.7):
        try:
            tokenizer, model, sentence_model = load_model(model_path)

            if sentence_model is not None:
                # Use sentence-transformers
                embeddings = sentence_model.encode([prompt])
  ...
  # Default Python inference code for code generation models
  defp default_code_inference_code do
    """
    import torch
    from transformers import AutoTokenizer, AutoModelForCausalLM
